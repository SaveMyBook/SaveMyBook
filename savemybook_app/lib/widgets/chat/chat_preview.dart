...
enum ChatPreviewAction { open, markRead, toggleMute, delete }
...
class _ChatPreviewState extends State<_ChatPreview> {
  ...
  Widget _header(AppColors c) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          UserAvatar(imageUrl: room.partner.avatarUrl, radius: 18),
  ...
  Widget _bubble(AppColors c, ChatMessage m) {
    ...
    if (m.isRecalled || m.kind == 'notice' || m.kind == 'book' || m.kind == 'reservation') {
      return Padding(

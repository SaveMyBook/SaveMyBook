...
const registerModels = ({ autoKeys = {}, uniqueKeys = {}, defaults = {} } = {}) => {
  ...
  for (const [table, keys] of Object.entries(uniqueKeys)) {
    UNIQUE_KEYS[table] = [...(UNIQUE_KEYS[table] ?? []), ...keys];
  }
  // 逐表合併，避免覆蓋掉既有欄位的預設值。
  for (const [table, values] of Object.entries(defaults)) {
    MODEL_DEFAULTS[table] = { ...(MODEL_DEFAULTS[table] ?? {}), ...values };
  }
};
...
const matchValue = (actual, expected) => {
  ...
  if (expected && typeof expected === 'object') {
    ...
    if ('not' in expected) {
      checks.push(expected.not === null ? actual !== null && actual !== undefined : !matchValue(actual, expected.not));
    }
    if ('lte' in expected) checks.push(actual != null && new Date(actual) <= new Date(expected.lte));
    if ('gte' in expected) checks.push(actual != null && new Date(actual) >= new Date(expected.gte));
    if ('lt' in expected) checks.push(actual != null && new Date(actual) < new Date(expected.lt));
    if ('gt' in expected) checks.push(actual != null && new Date(actual) > new Date(expected.gt));
    if ('contains' in expected) checks.push(String(actual ?? '').includes(String(expected.contains)));
...
const rank = (value) => (value instanceof Date ? value.getTime() : value ?? null);
...
const sortRows = (list, orderBy) => {
  const specs = (Array.isArray(orderBy) ? orderBy : orderBy ? [orderBy] : []).flatMap((o) => Object.entries(o));
  if (specs.length === 0) return list;
  return [...list].sort((a, b) => {
    for (const [field, direction] of specs) {
      const delta = compareBy(a[field], b[field]);
...
class FakePrisma {
  ...
  model(table) {
    ...
    const insert = (data) => {
      const key = AUTO_KEYS[table] ?? 'id';
      const row = {
        ...(MODEL_DEFAULTS[table] ?? {}), created_at: new Date(), updated_at: new Date(), ...data
      };
    ...
    return {
      ...
      findFirst: async (args = {}) =>
        project(sortRows(rows().filter((row) => matchWhere(row, args.where ?? {})), args.orderBy)[0] ?? null, args),
      findMany: async (args = {}) => paginate(
        sortRows(rows().filter((row) => matchWhere(row, args.where ?? {})), args.orderBy),
        args
      ...
      groupBy: async (args = {}) => {
        ...
        return [...groups.values()].map((items) => ({
          ...Object.fromEntries(args.by.map((field) => [field, items[0][field]])),
          ...(args._count && {
            _count: args._count === true
              ? items.length
              : Object.fromEntries(Object.keys(args._count).map((field) => [field, items.length]))
          })
        }));
      },
      create: async (args) => project(insert(args.data), args),
      createMany: async (args) => {
        const list = Array.isArray(args.data) ? args.data : [args.data];
        for (const data of list) insert(data);
  ...
  run(rawSql, values) {
    ...
    for (const { matcher, handler } of this.sqlHandlers) {
      if (typeof matcher === 'string' ? sql.includes(matcher) : matcher.test(sql)) {
        const result = handler(sql, values, this);
...
module.exports = { FakePrisma, registerModels, AUTO_KEYS, UNIQUE_KEYS, MODEL_DEFAULTS };

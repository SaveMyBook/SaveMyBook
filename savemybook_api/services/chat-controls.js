...
const mutedPairs = async (roomIds) => {
  ...
  const rows = await prisma.$queryRawUnsafe(
    `SELECT user_id, room_id FROM chat_room_mutes WHERE room_id IN (${roomIds.map(() => '?').join(',')})`,
    ...roomIds
  );
...
module.exports = {
  isAvailable, setMuted, mutedRoomIds, blockedUserIds, relation, assertCanMessage, recipientUnavailable,

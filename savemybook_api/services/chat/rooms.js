...
const findMine = async (roomId, myId, include) => {
  ...
  return {
    ...
    history: group ? history.floorOf(row) : null
  };
};
...
const shapeRoom = (room, myId, { meta, muted, blocked, aliasMap, groupNicknames, transfers }) => {
  const group = meta?.room_type === 'group';
  const newest = room.chat_messages[0] ?? null;
  const last = group && !history.isVisible(history.floorOf(meta), newest) ? null : newest;
...
const list = async (myId) => {
  const metaRows = await memberRooms(myId);
  ...
  const [rooms, muted, blocked, aliasMap] = await Promise.all([prisma.chat_rooms.findMany({
    where: { room_id: { in: [...metaById.keys()] } },
  ...
  return rooms
    .map((r) => shapeRoom(r, myId, { meta: metaById.get(r.room_id), muted, blocked, aliasMap, groupNicknames, transfers }))
